...
//    public Plot2D (double x, double y) {
//        this.xi = new double[]{0.0, x};
//        this.yi = new double[]{0.0, y};
//    }
    ...
    public SerializableImage plot0() throws IOException {
        ...
        int xspan = xm - x0;   // 600
        int yspan = ym - y0;   // 400
        double xmax = max(vals1);
        double xmin = min(vals1);
        double ymax = max(vals2);
        double ymin = min(vals2);
        ...
        for (int i = 0; i < vals1.length; i++) {
            int f = (int) ((vals1[i] - xmin) * xspan / (xmax - xmin));
            int h = (int) (((ymax - ymin) - (vals2[i] - ymin)) * yspan / (ymax - ymin));
            g2.drawString("o", x0 + f - 3, h + 14);
        }
        for (int i = 0; i < vals1.length - 1; i++) {
            int f = (int) ((vals1[i] - xmin) * xspan / (xmax - xmin));
            int f2 = (int) ((vals1[i + 1] - xmin) * xspan / (xmax - xmin));
            int h = (int) (((ymax - ymin) - (vals2[i] - ymin)) * yspan / (ymax - ymin));
            int h2 = (int) (((ymax - ymin) - (vals2[i + 1] - ymin)) * yspan / (ymax - ymin));
